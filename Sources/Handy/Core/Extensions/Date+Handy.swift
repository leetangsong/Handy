...
public extension HandyExtension where Base == Date{
    /// 日期转字符串
    ///
    /// - Parameter format: 格式
    /// - Returns: String
...
public extension HandyClassExtension where Base == Date{
    ...
    ///   - format: 格式
    /// - Returns: Date
    static func stringToDate (from: String , format: String) -> Date {
        Date.handy.displayFormatter.dateFormat = format;
    ...
    static func daysDifferent (date1: Date , date2: Date, component: Set<Calendar.Component>) -> DateComponents {
    ...
    //CST时间字符串转日期
    static func CSTTimeToDate (CST: String) -> Date? {
    ...
    ///   - year: 年数
    /// - Returns: (开始时间,结束时间)
    static func weekTimeBeginEnd(week: Int,year: Int) -> (startDate:Date,endDate:Date) {
        //week当中的一天
    ...
    ///   - formatter: 时间格式
    /// - Returns: (时间字符串)
    static func dateFormatString(timeStamp: String,formatter: String) -> String{
